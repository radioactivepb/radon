...
def mdn_config():
    ...
    cpdir("/ctx/blobs/mdn/fw", "/lib/firmware/amd/sof/community")
    cpdir("/ctx/blobs/mdn/tplg", "/lib/firmware/amd/sof-tplg")
...
def avs_config():
    ...
    cpdir("/ctx/blobs/avs/avs-topology/lib/firmware/intel/avs", "/lib/firmware/intel/avs")
    print_header("Enabling AVS driver")
    cpfile("/ctx/conf/avs/snd-avs.conf", "/etc/modprobe.d/snd-avs.conf")
...
def mtl_sof_config():
    ...
    cpfile("/ctx/conf/sof/mtl-sof.conf", "/etc/modprobe.d/mtl-sof.conf")

def hifi2_sof_config():
    print_header("Forcing SOF driver in debug mode")
    cpfile("/ctx/conf/sof/hifi2-sof.conf", "/etc/modprobe.d/hifi2-sof.conf")
...
def install_ucm():
    ...
    cpdir("/ctx/blobs/ucm/ucm2", "/usr/share/alsa/ucm2/")
    cpdir("/ctx/blobs/ucm/overrides", "/usr/share/alsa/ucm2/conf.d")
...
if __name__ == "__main__":
    ...
    print_header("Setting up keyboard...")
    ...
    if path_exists("/usr/bin/wireplumber"):
        ...
        cpfile("/ctx/conf/common/51-increase-headroom.conf", "/etc/wireplumber/wireplumber.conf.d/51-increase-headroom.conf")
    print_status("Audio setup finished!")
    print_status("Reboot to complete setup.")
